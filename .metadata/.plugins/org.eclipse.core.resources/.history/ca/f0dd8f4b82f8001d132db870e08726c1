...
<body>
	...
	<p>※Request (요청이 들어왔을때, Forward(페이지 요청)시에 딱 한번만 데이터를 유지함. URL=>Servlet, Servlet=>URK)</p>
	...
	<%
		pageContext.setAttribute("pageCtx", "KGH_PAGE_CTX");//페이지 내에서만 사용하는 객체
		request.setAttribute("req", "KGH_REQ");
		//Request => 1. getparameter (URL 또는 Form태그를 이용한경우 내부 태그를 name으로 구분하여 파라메터가 String으로 들어있음)
		//=> 2. Atrribute <= 어떤 요청을 통해서 오거나 Request가 Forward라는 기능으로 페이지 전환을 하는경우 Object 들어있음
		session.setAttribute("session", "KGH_SESSION");
		application.setAttribute("app", "KGH_APPv1.1");
	%>
	...
	<p>application : <%=application.getAttribute("app") %>
	 <jsp:forward page="Ex04_ScopeResult.jsp"/>  
